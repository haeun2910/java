...
public class Main {
    public static void main(String[] args) {
        // 내가 어떤 데이터베이스를 사용하는지를 정해주는 연결 주소
        String connectionString = "jdbc:sqlite:db.sqlite";
        try (Connection connection = DriverManager.getConnection(connectionString)){
            System.out.println("Connection Success!!!");

            // 데이터베이스에 SQL문(SQL Statement)를 전달한 준비를 한다
           ...
           """);
            statement.execute("""
            INSERT INTO user (username, password, first_name, last_name, email)
            values ('brad','bbb', 'Brad', 'Peter Brad', 'brad@gmail.com');
            """);
            statement.execute("""
            INSERT INTO user (username, password, first_name, last_name, email)
            values ('chad','cccc', 'Chad', 'Peter Chad', 'chad@gmail.com');
            """);
            statement.execute("""
            INSERT INTO user (username, password, first_name, last_name, email)
            values ('Dave','ddd', 'Dave', 'Peterdave', 'Dave@gmail.com');
            """);
...
//            statement.execute(String.format(insertSql, username, password));
            ...
            // 조회의 결과는 ResultSet으로 돌아온다. 
            ResultSet resultSet = statement.executeQuery("""
            SELECT * FROM user;
            """);
//            // ressultSet.next()를 호출하면 다음 줄로 이동한다.
//            resultSet.next();
...
//            statement.executeQuery(String.format(selectSql, input));
            ...
            // PreparedStament를 이용해 좀더 안전하게 SQL을 실행 할 수 있다.
