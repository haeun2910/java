...
public class D93UsingStream {
    public static void main(String[] args) {
        ...
        // filter(Pridicate): 전달받은 메서드릐 결과다 true인 원소들 만 남기는 중간작업
        Stream<String> nameStream1 = nameStream
                ...
                // 메서드 체이낭(method chaining)이라고 부른다
...
//        Stream<Integer> nameLengthStream = nameStream2.map(name -> name.length());

        // 중간 작업: 데이터를 정렬하자
        // sorted()
        // Stream의 데이터를 정렬한 새로운 Stream
        Stream<String> nameStream3 = nameStream2.sorted();
        //nameStream3.forEach(System.out::println);

        // sorted(Compartor)
        // Compartor를 사용해 데이터를 정렬한 새로운 Stream
